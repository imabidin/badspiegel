...
document.addEventListener("DOMContentLoaded", function () {
   ...
   */
  var userExists = false; // Whether the entered email belongs to existing user
  var currentEmail = ""; // Currently validated email address
  var originalFormTextContent = ""; // Original help text for form reset functionality
  var isProcessing = false; // Prevents concurrent AJAX requests
   ...
   */
  var formText = document.querySelector("#username_email_check_wrapper .form-text");
  if (formText) {
  ...
  function setupEventListeners() {
     ...
     */
    var emailInput = document.getElementById("username_email_check");
    if (emailInput) {
     ...
     */
    var form = document.getElementById("modern-login-form");
    if (form) {
      form.addEventListener("submit", function (e) {
        e.preventDefault();

        // Determine current state and route accordingly
        var loginSection = document.getElementById("username_email_valid");
        var registerSection = document.getElementById("username_email_invalid");
        if (loginSection && loginSection.classList.contains("show")) {
          handleLogin();
        } else if (registerSection && registerSection.classList.contains("show")) {
          handleRegistration();
        } else {
    ...
    if (continueBtn) {
      continueBtn.addEventListener("click", function (e) {
        ...
        var email = document.getElementById("username_email_check").value.trim();

        // Validate email presence
        if (!email) {
          showValidationMessage("Bitte geben Sie eine E-Mail-Adresse ein.", "error");
        ...
        if (!isValidEmail(email)) {
          showValidationMessage("Bitte geben Sie eine gültige E-Mail-Adresse ein (z.B. name@domain.de).", "error");
          document.getElementById("username_email_check").focus();
          return;
        }
  ...
  function showValidationMessage(message) {
    ...
    var formTextEl = document.querySelector("#username_email_check_wrapper .form-text");
    var emailInput = document.getElementById("username_email_check");
    if (formTextEl) {
      // Update message styling based on type
      var messageClass = type === "error" ? "text-danger" : type === "success" ? "text-success" : "text-muted";
      formTextEl.className = formTextEl.className.replace(/text-(muted|danger|success)/g, "");
  ...
  function clearValidationMessage() {
    ...
    if (formTextEl) {
      ...
      formTextEl.className = formTextEl.className.replace(/text-(danger|success)/g, "");
  ...
  function checkUserExists(usernameOrEmail) {
    ...
    fetch(ajax_object.ajax_url, {
      ...
      if (data.success) {
        ...
        if (data.data.valid_email === false) {
          showValidationMessage("Ungültige E-Mail-Adresse. Bitte überprüfen Sie die Eingabe.", "error");
        ...
        showValidationMessage("Fehler bei der Überprüfung. Bitte versuchen Sie es erneut.", "error");
        continueBtn.disabled = false;
        continueBtn.textContent = "Weiter";
      }
      ...
      showValidationMessage("Verbindungsfehler. Bitte versuchen Sie es erneut.", "error");
      continueBtn.disabled = false;
      continueBtn.textContent = "Weiter";
    });
    ...
    continueBtn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>';
    clearValidationMessage();
  }
  ...
  function showPasswordValidation(message) {
    var type = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "info";
    var formTextEl = document.querySelector("#username_email_valid .form-text");
    if (formTextEl) {
      var messageClass = type === "error" ? "text-danger" : "text-muted";
      formTextEl.className = formTextEl.className.replace(/text-(muted|danger)/g, "");
      formTextEl.classList.add(messageClass);
      formTextEl.textContent = message;
    }
  }
  ...
  function clearPasswordValidation() {
    ...
    if (formTextEl) {
      ...
      formTextEl.classList.add("text-muted");
      formTextEl.textContent = "Willkommen zurück! Bitte geben Sie Ihr Passwort ein.";
    }
  }
  ...
  function handleLogin() {
    ...
    if (password.length < 6) {
      document.getElementById("password").classList.add("is-invalid");
      showPasswordValidation("Das Passwort muss mindestens 6 Zeichen lang sein.", "error");
      return;
    }
  ...
  function validateLogin(email, password) {
    ...
    fetch(ajax_object.ajax_url, {
      ...
      if (data.success) {
        ...
        if (data.data.lockout) {
          ...
          showPasswordValidation(data.data.message || "Ungültige Anmeldedaten. Bitte überprüfen Sie Ihre Eingaben.", "error");
          continueBtn.disabled = false;
          continueBtn.textContent = "Anmelden";
          passwordInput.focus();
      ...
      showPasswordValidation("Verbindungsfehler. Bitte versuchen Sie es erneut.", "error");
      continueBtn.disabled = false;
      continueBtn.textContent = "Anmelden";
    });
    ...
    // Set immediate loading state
    continueBtn.disabled = true;
    continueBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Anmelden...';
    clearPasswordValidation();
  }
  ...
  function registerUser(email) {
    ...
    fetch(ajax_object.ajax_url, {
      ...
      if (data.success) {
        ...
        isProcessing = false;
        showRegistrationError(data.data.message || "Registrierung fehlgeschlagen. Bitte versuchen Sie es erneut.");
      }
    }).catch(function (error) {
      // Handle network errors
      isProcessing = false;
      console.error("Registration failed:", error);
      showRegistrationError("Verbindungsfehler. Bitte versuchen Sie es erneut.");
    });

    // Set immediate loading state
    continueBtn.disabled = true;
    continueBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Registrieren...';
  }
  ...
  function showRegistrationError(message) {
    ...
    var existingMessages = registerSection.querySelectorAll("#registration-error, #registration-success");
  ...
  function resetForm() {
    ...
    var messages = document.querySelectorAll("#registration-error, #registration-success");
  ...
  function startLockoutCountdown(remainingSeconds) {
    ...
    var _updateCountdown = function updateCountdown() {
      ...
      var timeString = minutes > 0 ? "".concat(minutes, ":").concat(seconds.toString().padStart(2, "0")) : "".concat(seconds);

      // Update button and message
      continueBtn.textContent = "Gesperrt (".concat(timeString, ")");
      showPasswordValidation("Account gesperrt. Verbleibende Zeit: ".concat(timeString, " Minuten"), "error");
