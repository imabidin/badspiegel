...
document.addEventListener("DOMContentLoaded", function () {
   ...
   */
  var getInputConstraints = function getInputConstraints(input) {
    var min = parseFloat(input.getAttribute("min"));
    var max = parseFloat(input.getAttribute("max"));
    var step = parseFloat(input.getAttribute("step"));
    return {
      min: isNaN(min) ? 0 : min,
      // Respect actual min="0" from HTML
   ...
   */
  var getCurrentValue = function getCurrentValue(input) {
    var current = parseFloat(input.value);
    if (!isNaN(current)) return current;
    var prev = parseFloat(input.dataset.prevValidValue);
    if (!isNaN(prev)) return prev;
    var original = parseFloat(input.dataset.originalValue);
   ...
   */
  var normalizeQuantity = function normalizeQuantity(input) {
    var allowEmpty = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    if (!(input !== null && input !== void 0 && input.getAttribute)) return null;
    var isEmpty = !input.value || input.value.trim() === "";
    if (isEmpty && allowEmpty) return null;
    var _getInputConstraints = getInputConstraints(input),
      min = _getInputConstraints.min,
      max = _getInputConstraints.max,
      step = _getInputConstraints.step;
    var value = isEmpty ? min : parseFloat(input.value);
   ...
   */
  var updateInputState = function updateInputState(input, isValid) {
    ...
    if (triggerChange) {
      ...
      var updateBtn = form === null || form === void 0 ? void 0 : form.querySelector('button[name="update_cart"]');
   ...
   */
  var handleEmptyInput = function handleEmptyInput(input) {
  ...
  document.body.addEventListener("focusout", function (event) {
    ...
    var newValue = normalizeQuantity(input);
    var isValid = !isNaN(newValue) && newValue === parseFloat(input.value);
    var hasChanged = newValue !== parseFloat(input.dataset.prevValidValue);
  ...
  document.body.addEventListener("click", function (event) {
    ...
    var input = (_button$closest = button.closest(".quantity")) === null || _button$closest === void 0 ? void 0 : _button$closest.querySelector(".qty");
    if (!input) return;
    var _getInputConstraints2 = getInputConstraints(input),
      min = _getInputConstraints2.min,
      step = _getInputConstraints2.step;
    var currentValue = getCurrentValue(input);
    var newValue = button.classList.contains("plus") ? currentValue + step : Math.max(min, currentValue - step);
  ...
  document.body.addEventListener("input", function (event) {
    ...
    var isValid = input.value.trim() !== "" && !isNaN(parseInt(input.value, 10));
    updateInputState(input, isValid, false);
  });
});
...
var debug = function debug() {
  if (CONFIG.DEBUG) {
    ...
    (_console = console).log.apply(_console, ["[CHECKOUT-SPINNER]"].concat(args));
  }
};
...
function handleCheckoutClick(event) {
  ...
  var originalContent = checkoutButton.innerHTML;
  var originalHref = checkoutButton.href;
...
/*!************************!*\
  ...
  \************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _scss_cart_scss__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./scss/cart.scss */ "./assets/scss/cart.scss");
/* harmony import */ var _js_cart_quantity__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./js/cart/quantity */ "./assets/js/cart/quantity.js");
/* harmony import */ var _js_cart_spinner__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./js/cart/spinner */ "./assets/js/cart/spinner.js");
// cart.js
