...
<p class="woocommerce-result-count" <?php echo (empty($orderedby) || 1 === intval($total)) ? '' : 'role="alert" aria-relevant="all" data-is-sorted-by="true"'; ?>>
	...
	if (1 === intval($total)) {
		...
		$orderedby_placeholder = empty($orderedby) ? '%2$s' : '<span class="visually-hidden-focusable">%2$s</span>';
		...
		$orderedby_placeholder = empty($orderedby) ? '%4$s' : '<span class="visually-hidden-focusable">%4$s</span>';
